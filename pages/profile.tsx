...
const ProfilePage = () => {
  ...
  const handleSubmit = async e => {
    ...
    setError('')
    setSuccess('')
    // TODO: Panggil endpoint update profile (akan dibuat)
    // Validasi, update, dsb
    setLoading(false)
    ...
    <div className="max-w-xl mx-auto my-10 bg-white dark:bg-gray-900 rounded-2xl shadow-lg p-8">
      ...
      <div className="flex items-center gap-4 mb-6">
        {currentUser.avatar_url ? (
          <img src={currentUser.avatar_url} alt="Avatar" className="w-16 h-16 rounded-full border-2 border-blue-400" />
        ) : (
      ...
      {!editMode ? (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-4">
          <div>
            ...
            <input type="text" name="name" value={form.name} onChange={handleChange} className="input-modern w-full" required />
          </div>
          <div>
            <label className="block mb-1 font-medium">Email</label>
            <input type="email" name="email" value={form.email} onChange={handleChange} className="input-modern w-full" required />
          </div>
          <div>
            <label className="block mb-1 font-medium">Nomor HP</label>
            <input type="text" name="phone" value={form.phone} onChange={handleChange} className="input-modern w-full" required placeholder="Format: +6281234567890" />
          </div>
          <div>
            <label className="block mb-1 font-medium">Password Lama</label>
            <input type="password" name="password" value={form.password} onChange={handleChange} className="input-modern w-full" placeholder="Isi jika ingin ganti password" />
          </div>
          <div>
            <label className="block mb-1 font-medium">Password Baru</label>
            <input type="password" name="newPassword" value={form.newPassword} onChange={handleChange} className="input-modern w-full" placeholder="Isi jika ingin ganti password" />
          </div>
